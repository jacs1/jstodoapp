...
var todoApp = {
	...
	//this method is called when the user submits a new task, it creates the task and passes it to the append_task method
	create_task: function(text){
		//first it makes a clone of the todoItem using the beget() method we defined above
		var task = Object.beget(todoItem);
		//next it sets the task using newTask's setTaskText() method
		task.setTaskText(text);
		//pass the task to the append_task method so that it get's added to the DOM
	...
	append_task: function(task){
		...
		if(task.task_name.length!==0){
			...
			this.todo_tasks().appendChild(task.render());
		}
	}
}
...
window.onload = function(){
	...
	//this function will create a new todo task using the text entered into the fild
	//it responds to a mouse click and an 'enter' keypress
	var input_event = function(e){
		//keyCode of 'enter' is 13, keyCode of leftclick is 0
		if (e.keyCode===13||e.keyCode===0){
			//pass the input's value into the create_task method that will generate and append a new todo task
			todoApp.create_task(user_input.value);
	...
	button.onclick = input_event;
	user_input.onkeypress = input_event;
}	